...
\setmainjfont{Source Han Serif SC}[
  YokoFeatures={JFM=CCTMod}
]
\setsansjfont{Source Han Sans SC}[
  YokoFeatures={JFM=CCTMod}
]
...
\newjfontfamily\fontSansHans{Source Han Sans SC}[
  YokoFeatures={JFM=CCTMod}
]
...
\tcbset{colback=white,fonttitle=\bfseries}
...
\NewDocumentCommand{\InvisiblePart}{m}{
  \clearpage
  \refstepcounter{part}
...
\NewDocumentCommand{\TitleFormatSectionWithNumber}{}{
  \titlespacing{\section}{0pt}{*4}{*1.5}
  \titleformat{\section}[hang]
  {%format
  \sffamily\gtfamily\bfseries\large%
  }{%label
  \thesection
  }{%sep
  0.25em
...
\NewDocumentCommand{\RestorePageLayout}{}{
  \fancypagestyle{plain}{
    \ifoptionfinal {
      \fancyhead[C]{\small\ShowEntriesForPage{\value{page}}}
    } {
      \fancyhead[C]{\small DRAFT}
    }
    \fancyhead[LO,RE]{\small\sffamily\gtfamily\leftmark}
    \fancyhead[RO,LE]{\thepage}
    \fancyfoot{}
  }
  \pagestyle{plain}

  \titlespacing{\part}{0pt}{-14pt}{0pt}
  ...
  \titlespacing*{\subsection}{0pt}{0.25em plus 0.05em minus 0.05em}{0.125em plus 0.025em minus 0.025em}
...
% fancy-thumb
\setlength{\unitlength}{16mm}
\newcommand{\blob}{\rule[-.25\unitlength]{2\unitlength}{.5\unitlength}}
\newcommand\rblob[1]{\thepage
\begin{picture}(0,0)
\put(0.25,-\value{section}){\blob}
\put(0.35,-\value{section}){\large\fontseries{eb}\sffamily\gtfamily\rlap{\raisebox{-0.4em}[0pt][0pt]{\textcolor{white}{#1}}}}
\end{picture}}
\newcommand\lblob[1]{%
\begin{picture}(0,0)
\put(-2.25,-\value{section}){\blob}
\put(-0.35,-\value{section}){\large\fontseries{eb}\sffamily\gtfamily\llap{\raisebox{-0.4em}[0pt][0pt]{\textcolor{white}{#1}}}}
\end{picture}%
\thepage}
\newcounter{line}
...
\NewDocumentEnvironment{MainGroup}{m m m}{
  \int_compare:nNnT {\thesection} > {0} {
    \clearpage%
  }

  \fancypagestyle{plain}{
    \fancyhead[LO,RE]{\small\sffamily\gtfamily\leftmark}
    \fancyhead[RO]{\rblob{\tl_item:nn{#1}{1}}}
    \fancyhead[LE]{\lblob{\tl_item:nn{#1}{1}}}
    \fancyfoot{}
  }
  \pagestyle{plain}
  ...
  \titleformat{\section}[hang]
  {%format
    \bfseries\sffamily\gtfamily\huge%
  }{%label
    \thesection%
  }{%sep
    0.5em
  ...
  \section[#1（#2）]{#1{\large （#2）}}\label{group-#1}%
  \markboth{\thesection #1}{}
  \begin{tcolorbox}[title=韵部内諧聲部一覽,fonttitle=\sffamily\gtfamily]
    \small
    ...
    \begin{multicols}{6}
      \tl_map_inline:nn {#3} {
        \noindent
        \tl_item:nn {##1} {2}
        %\dotfill\pageref{main-part-\tl_item:nn {##1} {1}}
        \nobreakspace
        \quelle{
    ...
    \end{multicols}
  \end{tcolorbox}
  \vspace{1em plus 1em minus 0.2em}
}{%
}
...
\NewDocumentCommand{\IndexHeader}{m m}{
  \begin{tcolorbox}[
    halign=center,
    fontupper=\bfseries,
...
\NewDocumentCommand{\GroupIndexEntry}{m m m}{
  \begin{tcolorbox}[title={\textbf{\ref{group-#1} #1（#2）} \textmd{(p. \pageref{group-#1}--)}}]
    \small
    \setlength{\columnsep}{0.5em}
    \setlength{\columnseprule}{0.4pt}
    \begin{multicols}{6}
...
\NewDocumentEnvironment{RadicalSection}{m m}{
  \IndexHeader{{\footnotesize #1}\\#2部}{
    \label{index-radical-section-#1-#2}%
...
\NewDocumentCommand{\RadicalEntryMissing}{}{
  \begin{center}
    ---
  \end{center}
}
\NewDocumentCommand{\RadicalEntry}{m m m m}{
  ...
  #4\space\hfill\quelle{\pageref{entry-#1}}
  %\par
}
...
\NewDocumentEnvironment{ReadingSection}{m o}{
  \IndexHeader{
    \fReadingIndexLabel #1
    
    \tl_if_empty:nF {#2} {
      {\\\footnotesize\mdseries #2}
    }
  }{\label{index-reading-section-#1}}
  \str_clear:N \l_zqdict_index_subsection_label_str
}{}
...
\NewDocumentEnvironment{PartRadicals}{}{
  \setlength{\columnsep}{2.25em}
  \setlength{\columnseprule}{0.4pt}
  \begin{multicols}{6}
}{
...
\NewDocumentCommand{\PartRadicalEntry}{m m}{
  \noindent
  \hfill#2\hspace{1em}{\fPartRadix#1}
  \par
}
\ExplSyntaxOff
