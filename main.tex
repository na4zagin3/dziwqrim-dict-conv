...
\cs_set:Nn \_zqdict_xr_entry:nn {
  \tl_build_gput_right:Nx \g_zqdict_entries_tl {{{#1}{#2}{\tl_use:N{\g_zqdict_current_section_tl}}}}
}
...
\cs_generate_variant:Nn \_zqdict_entry_get_section:n { o , x , V }
\cs_generate_variant:Nn \_zqdict_section_get_number:n { o , x , V }
\cs_generate_variant:Nn \_zqdict_section_get_part:n { o , x , V }
...
\NewDocumentCommand{\RecordEntry}{m}{
  \iow_shipout_x:cn { @auxout } {
    \token_to_str:N \ExplSyntaxOn
    \token_to_str:N \_zqdict_xr_entry:nn { #1 } { \int_eval:n { \value { page } } }
    \token_to_str:N \ExplSyntaxOff
  }
}
\AtEndDocument {
...
\str_new:N \l_zqdict_item_part_str
\str_new:N \l_zqdict_current_part_str

\cs_set:Nn \_zqdict_render_entry_for_the_current_page:n {
  \tl_set:Nx \l_tmpa_tl {
    \_zqdict_entry_get_section:n {#1}
  }
  \str_set:Nx \l_zqdict_item_part_str {
    \_zqdict_section_get_part:V \l_tmpa_tl
  }
  \int_compare:nNnT {\_zqdict_entry_get_page:n {#1}} = { \value {page} } {
    \str_if_eq:VVTF \l_zqdict_item_part_str \l_zqdict_current_part_str {
      \hspace{0.25em}
    } {
      \str_if_eq:VnF \l_zqdict_current_part_str {} {
        \quad
      }
      \textbf{
        {
        ...
        \str_use:N \l_zqdict_item_part_str
      }
      \hspace{0.5em}
      \str_set:NV \l_zqdict_current_part_str \l_zqdict_item_part_str
    }
    \_zqdict_entry_get_headword:n {#1}
  }
}
\NewDocumentCommand{\ShowEntriesForTheCurrentPage}{}{
  \str_clear:N \l_zqdict_current_part_str
  \tl_map_function:NN \g_zqdict_entries_tl \_zqdict_render_entry_for_the_current_page:n
}
