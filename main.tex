...
\setfontface\fSemanticLabel{Gentium Plus}
...
\geometry{top=1.5cm,headheight=18pt,headsep=2pt}
...
\titlespacing*{\section}{0pt}{1em plus 40em minus 0.2em}{0pt}
...
\newenvironment{Sense}{\begin{itemize}[itemsep=0.125em,labelindent=0.5em,itemindent=-1em,leftmargin=1.5em]}{\end{itemize}}
...
\NewDocumentCommand{\SikrokEntry}{m m o m}{
  \noindent%
  \IfNoValueTF{#3}{%
    #2\hfill #4\hfill\pageref{entry-#1}%
  }{%
    #2\hfill #4（#3）\pageref{entry-#1}%
  }%
  \par
}
...
\NewDocumentCommand{\RadicalEntry}{m m m m}{
  \noindent%
  #2\hfill #3\hfill #4\pageref{entry-#1}%
...
\NewDocumentCommand{\ReadingEntry}{m m m}{
  \noindent%
  \makebox[4em minus 2em][l]{#2} #3 \hfill\pageref{entry-#1}%
  \par
}
